...
async def on_message(message):
    ...
    if message.content == "r>quote": # i have no idea how this works
        ...
        try:
            ...
            for role in original_message.author.roles.reverse():
...
@bot.tree.command(name="view-achievements", description="View all the achievements!")
async def viewachievements(interaction: discord.Interaction):
    await interaction.response.defer()
    allachievements = loadData("achievements")
    if allachievements == "":
        await interaction.edit_original_response(content="An error occurred while loading achievement data.")
    ...
    embed = discord.Embed(title="Achievements", description="All the achievements.", color=discord.Color.random())
    for achievementid, achievementinfo in allachievements.items():
        if str(interaction.user.id) in playerachievements and achievementid in playerachievements[str(interaction.user.id)]:
            achievedprefix = "[Owned]"
        else:            
            achievedprefix = ""
        name = achievementinfo.get("name", f"ID {achievementid}")
        description = achievementinfo.get("description", "No description provided.")
        embed.add_field(name=f"{achievedprefix} {name}", value=description, inline=False)
    await interaction.edit_original_response(embed=embed)

@bot.tree.command(name="award-achievement", description="Award an achievement to a user!")
@app_commands.describe(user="The user to award the achievement to.", achievement="The achievement to award.")
